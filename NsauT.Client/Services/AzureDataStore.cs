...
namespace NsauT.Client.Services
{
    public class AzureDataStore : IDataStore<TimetableItem>
    {
        HttpClient client;
        IEnumerable<TimetableItem> items;
        ...
        public async Task<IEnumerable<TimetableItem>> GetItemsAsync(bool forceRefresh = false)
        {
            if (forceRefresh && IsConnected)
            {
                ...
                items = await Task.Run(() => JsonConvert.DeserializeObject<IEnumerable<TimetableItem>>(json));
            }
        ...
        public async Task<TimetableItem> GetItemAsync(string id)
        {
            if (id != null && IsConnected)
            {
                ...
                return await Task.Run(() => JsonConvert.DeserializeObject<TimetableItem>(json));
            }
